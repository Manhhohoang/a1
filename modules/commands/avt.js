...
case "chuky": {
api.unsendMessage(handleReply.messageID)
return api.sendMessage(`✔Đã tìm thấy chữ ký: ${event.body}

[!] Vui lòng phản hồi tin nhắn này để nhập màu
(Bằng tiếng anh)`,event.threadID,(e, info)=> {
global.client.handleReply.push({
            name: this.config.name, maso: handleReply.maso , chunen: handleReply.chunen,
chuky: event.body,
            messageID: info.messageID,
            author: event.senderID,
            type: "mau"
...
return api.sendMessage({body: `Đây là avt của ${name}
Mã số nhân vật: ${handleReply.maso}
Chữ nền: ${handleReply.chunen}
Chữ ký: ${handleReply.chuky}
Màu: ${event.body} `, attachment: createReadStream(path),
mentions: [{ tag: name, id : event.senderID}] }, event.threadID, () =>
unlinkSync(path), handleReply.messageID);
...
Nhập ${global.config.PREFIX}avt để xem mã số
