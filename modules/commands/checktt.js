...
module.exports.run = async function ({ args, api, event, Currencies, getText }) {
    ...
    if (args[0] == "all") {
        ...
        return api.sendMessage(getText("mention", infoUser.name, rank, infoUser.exp), event.threadID,event.messageID);
    }
    else {
        var storage = [], exp = [];
        for (const value of data.userInfo) storage.push({"id" : value.id, "name": value.name});
        ...
        const infoUser = exp[rank - 1];
        return api.sendMessage(getText("myself", rank, infoUser.exp), event.threadID,event.messageID);
    }
}